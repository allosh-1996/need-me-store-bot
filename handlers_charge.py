...
async def charge_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """entry_point للـ ConversationHandler — بدون @rate_limit عشان ما يكسر الـ state."""
...
async def charge_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /charge command — يرد بزر لفتح الـ ConversationHandler.
    لا يستدعي charge_start مباشرة لأن الـ states لن تعمل خارج الـ conversation.
    """
...
async def charge_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(x in text for x in ["ابدأ", "Start", "/start"]):
...
async def charge_txhash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _notify_admins_charge(context, user, charge_id, method, amount_usd, tx_hash=tx_hash)
    await update.message.reply_text(
...
async def charge_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _notify_admins_charge(context, user, charge_id, method, amount_usd, proof_type="photo")
    await update.message.reply_text(
        f"✅ *{t('charge_submitted', lang)}*\n\n—————————————————\n\n"
        f"🔖 Request ID: `#{charge_id}`\n💵 ${amount_usd:.2f}\n\n"
        f"_{t('charge_pending', lang)}_\n\n—————————————————\n",
...
async def _notify_admins_charge(context, user, charge_id, method, amount_usd,
