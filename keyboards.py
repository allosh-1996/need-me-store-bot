...
def main_menu():
    keyboard = [
        [
            ...
            InlineKeyboardButton("ايميلات", callback_data="emails_menu"),
        ],
        [
...
def products_menu(products):
    keyboard = []
    for p in products:
        stock_icon = "متوفر" if p['stock'] else "غير متوفر"
        price_text = f"${p['price_usd']}" if p['price_usd'] else ""
        keyboard.append([InlineKeyboardButton(
            f"{p['name']}  —  {price_text}  ({stock_icon})",
...
def payment_method_menu(product_id, currency):
    keyboard = [
        ...
        [InlineKeyboardButton("من رصيدي", callback_data=f"confirm_buy_{product_id}")],
...
def persistent_menu():
    ...
    return ReplyKeyboardMarkup(
        ...
        input_field_placeholder="اكتب او اختر من القائمة..."
    )
