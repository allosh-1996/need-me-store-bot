...
def init_db():
    ...
    _q(conn, """CREATE TABLE IF NOT EXISTS users (
    ...
    _q(conn, """CREATE TABLE IF NOT EXISTS products (
    ...
    _q(conn, """CREATE TABLE IF NOT EXISTS stock_items (
        id         INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    _q(conn, """CREATE TABLE IF NOT EXISTS orders (
        id             INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    _q(conn, """CREATE TABLE IF NOT EXISTS charge_requests (
        id         INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id    INTEGER NOT NULL,
        username   TEXT,
        full_name  TEXT,
    ...
    _q(conn, """CREATE TABLE IF NOT EXISTS proxy_orders (
        id               INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id          INTEGER NOT NULL,
        username         TEXT,
        full_name        TEXT,
    ...
    _q(conn, """CREATE TABLE IF NOT EXISTS appsflyer_orders (
        id           INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id      INTEGER NOT NULL,
        username     TEXT,
        full_name    TEXT,
...
def set_user_lang(user_id: int, lang: str):
    conn = get_conn()
    _q(conn, "UPDATE users SET lang = ? WHERE id = ?", lang, user_id)
    conn.commit()
...
def block_user(user_id: int):
    conn = get_conn()
    _q(conn, "UPDATE users SET blocked = 1 WHERE id = ?", user_id)
    conn.commit()
...
def add_product(name: str, description: str, price_usd: float, price_syp: float,
    ...
    if lines:
        _bulk_add_stock(pid, lines)
    return pid


def delete_product(product_id: int):
    conn = get_conn()
    _q(conn, "UPDATE products SET active = 0 WHERE id = ?", product_id)
    conn.commit()
...
def get_categories(platform: str = None) -> list:
    ...
    if platform:
        cur = _q(conn,
            "SELECT DISTINCT category FROM products WHERE active = 1 AND platform = ? AND category IS NOT NULL ORDER BY category",
            platform)
    else:
        cur = conn.execute(
...
def pop_stock_item(product_id: int) -> tuple:
    ...
    try:
        row = _q(conn,
            "SELECT id, content FROM stock_items WHERE product_id = ? AND sold = 0 ORDER BY id ASC LIMIT 1",
            product_id).fetchone()
        if not row:
            conn.execute("ROLLBACK")
            return None, 0
...
def set_stock_items(product_id: int, lines: list):
    conn = get_conn()
    conn.execute("BEGIN")
    try:
        _q(conn, "DELETE FROM stock_items WHERE product_id = ? AND sold = 0", product_id)
        if lines:
            conn.executemany(
                "INSERT INTO stock_items (product_id, content) VALUES (?, ?)",
                [(product_id, line) for line in lines])
...
def create_order_atomic(user_id: int, username: str, full_name: str,
    ...
    try:
        ...
        _q(conn, "UPDATE users SET balance = balance - ? WHERE id = ?", price_usd, user_id)
        cur = _q(conn, """
            INSERT INTO orders
...
def get_order(order_id: int) -> dict | None:
    return _fetchone(_q(get_conn(), "SELECT * FROM orders WHERE id = ?", order_id))
...
def update_order_delivered_item(order_id: int, item: str):
    conn = get_conn()
    _q(conn, "UPDATE orders SET delivered_item = ? WHERE id = ?", item, order_id)
    conn.commit()
...
def get_charge_request(charge_id: int) -> dict | None:
    return _fetchone(_q(get_conn(), "SELECT * FROM charge_requests WHERE id = ?", charge_id))


def get_charges_recent(limit: int = 100) -> list:
    return _fetchall(_q(get_conn(),
...
def confirm_charge(charge_id: int) -> dict | None:
    ...
    conn.execute("BEGIN")
    try:
        charge = _fetchone(_q(conn, "SELECT * FROM charge_requests WHERE id = ?", charge_id))
        if not charge or charge["status"] != "pending":
            conn.execute("ROLLBACK")
            return None
        _q(conn, "UPDATE charge_requests SET status = 'confirmed' WHERE id = ?", charge_id)
...
def reject_charge(charge_id: int):
    conn = get_conn()
    _q(conn, "UPDATE charge_requests SET status = 'rejected' WHERE id = ?", charge_id)
    conn.commit()
...
def get_proxy_orders(status: str = "") -> list:
    ...
    return _fetchall(conn.execute("SELECT * FROM proxy_orders ORDER BY created_at DESC"))


def update_proxy_order_status(order_id: int, status: str):
    conn = get_conn()
    _q(conn, "UPDATE proxy_orders SET status = ? WHERE id = ?", status, order_id)
    conn.commit()
...
def get_appsflyer_order(order_id: int) -> dict | None:
    return _fetchone(_q(get_conn(), "SELECT * FROM appsflyer_orders WHERE id = ?", order_id))
...
def get_appsflyer_orders(status: str = "") -> list:
    ...
    return _fetchall(conn.execute("SELECT * FROM appsflyer_orders ORDER BY created_at DESC"))


def update_appsflyer_order_status(order_id: int, status: str):
    conn = get_conn()
    _q(conn, "UPDATE appsflyer_orders SET status = ? WHERE id = ?", status, order_id)
    conn.commit()
...
def get_all_notifications_full() -> list:
    ...
    """)):
        result.append({**row, "type": "charge", "icon": "💰",
    ...
    """)):
        result.append({**row, "type": "order", "icon": "🛍️",
    ...
    """)):
        result.append({**row, "type": "appsflyer", "icon": "🎮",
...
def get_users_for_dashboard() -> list:
    return get_users_with_balances()
